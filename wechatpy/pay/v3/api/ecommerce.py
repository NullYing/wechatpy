...
class WeChatEcommerce(BaseWeChatPayAPI):
    ...
    def trade_bill(
        ...
        return self._get("/v3/bill/tradebill", params=query)
    ...
    def fund_flow_bill(
        ...
        return self._get("/v3/bill/fundflowbill", params=query)
    ...
    def profit_sharing_bill(self, bill_date, tar_type=None, sub_mchid=None):
        ...
        :param tar_type: 压缩类型，不填则以不压缩的方式返回数据流
               GZIP: GZIP格式压缩，返回格式为.gzip的压缩包账单
        :param sub_mchid:  子商户号 若商户是直连商户无需填写该字段
        :return: 返回的结果数据
        """
        query = {
            "bill_date": bill_date,
        ...
        return self._get("/v3/profitsharing/bills", params=query)
    ...
    def eco_fund_flow_bill(
        ...
        :param bill_date: 账单日期，格式yyyy-MM-DD，仅支持三个月内的账单下载申请
        :param account_type:  资金账户类型，本接口只支持填ALL
        :param tar_type: 压缩类型，不填则以不压缩的方式返回数据流
               GZIP: GZIP格式压缩，返回格式为.gzip的压缩包账单
        :param algorithm: 账单文件加密算法
        :return: 返回的结果数据
        """
        query = {
            "bill_date": bill_date,
        ...
        return self._get("/v3/ecommerce/bill/fundflowbill", params=query)
    ...
    def sub_mch_fund_flow_bill(self, sub_mchid, bill_date, account_type, algorithm='AEAD_AES_256_GCM', tar_type=None):
        ...
        return self._get("/v3/bill/sub-merchant-fundflowbill", params=query)

    def download_bill(self, url):
        return self._get(url, skip_check_signature=True)